...
const FlaggerTimesheetEditScreen = ({ route, navigation }: Props) => {
...
const loadActiveJobs = async () => {
  try {
    const res = await apiClient.get('/api/job-phases/active');
    console.log("🔍 Backend Job Data:", res.data[0]); // Check if 'job_name' exists in the first item
    setAllActiveJobs(res.data || []);
    setJobModalVisible(true);
  } catch (err) {
    Alert.alert("Error", "Could not load active jobs");
  }
};
...
const handleSelectJob = (job: any) => {
  setTimesheet(prev => {
    ...
    return {
      ...
      data: {
        ...prev.data,
        job_name: job.job_name, // 👈 Store the job name here
        job: {
          ...prev.data.job,
  ...
  // Update the phases available for this new job
  setJobPhaseCodes(job.phase_codes || []);
  setSelectedPhases([]); // Reset selected phases as they belong to the old job
  ...
  if (d.job) {
    setTimesheet(prev => {
      ...
      return {
        ...
        data: {
          ...prev.data,
          job: d.job // 👈 This sets the job_code back into the UI state
        }
      };
    });
    
    // 2. Update the available phase codes for that restored job
    if (d.job.phase_codes) {
        setJobPhaseCodes(d.job.phase_codes);
    }
  }
  if (d.ticketsLoads) {
 ...
 if (Array.isArray(d.dumping_sites)) {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        const userDataRaw = await AsyncStorage.getItem('user'); // Use the key your app uses for login (e.g., 'user', 'profile')
        if (userDataRaw) {
          const loggedInUser = JSON.parse(userDataRaw);
          
          // Check for common key variations (firstname vs first_name)
          const first = loggedInUser.first_name || loggedInUser.firstname || '';
          const last = loggedInUser.last_name || loggedInUser.lastname || '';
          const middle = loggedInUser.middle_name || loggedInUser.middlename || '';
          
          const fullName = `${first} ${middle} ${last}`.replace(/\s+/g, ' ').trim();
          setForemanName(fullName);
        }
      } catch (storageErr) {
        console.warn("Failed to load user from storage", storageErr);
      }
try {
...
linked_tickets: selectedTicketIds,
      ...
      console.log('Auto-saved with Job Code:', timesheet.data?.job?.job_code);
...
console.log('🎫 DRAFT CONTENTS:', JSON.parse(verifyDraft!).data.ticketsLoads);
  ...
  return () => {
    ...
    timesheet, // 👈 ADD THIS: Triggers save when job_code or job_name changes
  employeeHours,
  equipmentHours,
  materialHours,
  vendorHours,
...
const autoSaveDraft = useCallback(async () => {
  ...
  try {
    ...
    // Ensure the job object is explicitly included from the current state
    const dataWithPhases = { 
      ...processedData, 
      selectedPhases: selectedPhases || [],
      job: timesheet?.data?.job // 👈 Force the latest job data into the save object
    };
    
    const draft = { timesheetId, data: dataWithPhases, status: 'Pending', savedAt: Date.now() };
    
    // IMPORTANT: Ensure this key matches your restore key exactly
    const key = `@autoSave_timesheet_${timesheetId}`; 
    
    await AsyncStorage.setItem(key, JSON.stringify(draft));
    console.log('✅ AUTOSAVE SUCCESS - Job Code Saved:', timesheet?.data?.job?.job_code);
  } catch (e) {
    console.error('💥 AUTOSAVE ERROR:', e);
  }
// ADD 'timesheet' TO THIS ARRAY BELOW 
}, [timesheetId, loading, isSubmitting, selectedPhases, buildUpdatedData, timesheet]);
useFocusEffect(
  useCallback(() => {
    let isActive = true;
    return () => {
...
<View style={styles.infoItem}>
  ...
  <Text style={styles.infoValue}>{foremanName || 'Not Logged In'}</Text>
</View>  
<WeatherLocationCard
...
<View style={{ flexDirection: 'row', gap: 10, marginTop: 10 }}>
  ...
  >
    <Text style={styles.phaseSelectorText}>
      Job: {timesheet.data?.job?.job_code ?? 'Select'} ▼
    </Text>
  </TouchableOpacity>

  {/* EXISTING PHASE CODE BUTTON */}
  <TouchableOpacity 
...
<Modal visible={jobModalVisible} animationType="slide" transparent>
  <View style={styles.modalBackdrop}>
    <View style={styles.modalContainer}>
      <Text style={styles.modalTitle}>Select Active Job</Text>
...
{allActiveJobs
  .filter(j => 
    j.job_code.toLowerCase().includes(jobSearch.toLowerCase()) || 
    (j.job_description && j.job_description.toLowerCase().includes(jobSearch.toLowerCase()))
  )
  .map((job) => {
    const isSelected = timesheet.data?.job?.job_code === job.job_code;

    return (
      <TouchableOpacity 
        key={job.job_code} 
        style={[styles.phaseItem, isSelected && { backgroundColor: '#f0f7ff' }]} 
        onPress={() => handleSelectJob(job)}
      >
        <View style={{ flex: 1 }}>
          {/* Display job_description instead of job_name */}
          <Text style={[styles.phaseText, { fontWeight: '700' }]}>
        ...
        <View style={[
          styles.checkbox, 
          isSelected && { backgroundColor: '#137fec', borderColor: '#137fec' }
        ]}>
          {isSelected && <Text style={[styles.tick, { color: '#fff' }]}>✓</Text>}
        </View>
      </TouchableOpacity>
    );
  })}
...
export default FlaggerTimesheetEditScreen;
