...
const PETicketList = () => {
    ...
    // ✅ CATEGORIZATION LOGIC (Synced with Supervisor Screen)
    const sections = useMemo(() => {
        ...
        tickets.forEach((t) => {
            ...
            if (cat.toLowerCase().includes('dump')) cat = 'Dumping Sites';

            // Fallback categorization based on material keywords
            if (cat === "Unspecified" && t.material) {
                const mat = t.material.toLowerCase();
                if (mat.includes('dump') || mat.includes('landfill')) {
                    cat = 'Dumping Sites';
                } else if (mat.includes('haul') || mat.includes('truck') || mat.includes('delivery')) {
                    cat = "Trucking";
                } else if (mat.includes('rock') || mat.includes('dirt') || mat.includes('sand') || mat.includes('stone')) {
                    cat = "Materials";
                }
            }
    ...
    const getCategoryIcon = (title: string) => {
        switch (title) {
            ...
            case 'Trucking': return 'bus-outline';
            case 'Dumping Sites': return 'trash-outline';
...
const styles = StyleSheet.create({
    ...
    tableInput: { fontSize: 12, color: '#333', padding: 8, height: '100%', width: '100%' },
