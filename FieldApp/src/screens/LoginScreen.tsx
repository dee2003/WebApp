...
// Don't forget to install this library: npm install @react-native-async-storage/async-storage
// --------------------
...
const LoginScreen: React.FC<Props> = ({ navigation }) => {
  ...
  // --- NEW FUNCTION TO LOAD SAVED EMAIL/PREFERENCE ---
  const loadRememberedEmail = async () => {
    try {
      const rememberedEmail = await AsyncStorage.getItem(STORAGE_KEYS.EMAIL);
      const rememberMePref = await AsyncStorage.getItem(STORAGE_KEYS.REMEMBER_ME);

      const shouldRemember = rememberMePref === 'true';

      if (rememberedEmail && shouldRemember) {
        setEmail(rememberedEmail);
        setIsRememberMeChecked(true);
      } else {
        // Ensure the checkbox state is consistent with the preference
        setIsRememberMeChecked(shouldRemember);
      }
    } catch (e) {
      console.log('Failed to load login data:', e);
    }
  };
...
const handleLogin = async () => {
  ...
  try {
    ...
    if (!response.data.access_token || !isValidRole(backendRole)) {
      Alert.alert('Login Failed', 'Invalid role or server response.');
      return;
    }
    ...
    // --- Login and set Axios token globally ---
    await login(userData, response.data.access_token);

    // --- Debug: Verify header is set ---
    console.log('🔑 Axios Auth Header:', apiClient.defaults.headers.common.Authorization);

    // --- Navigate to home/dashboard ---
    // navigation.replace("Home"); // uncomment if using react-navigation
            ...
            <View style={styles.rememberMeContainer}>
              ...
              >
                <View style={[styles.checkbox, isRememberMeChecked && styles.checkboxChecked]}>
                  {isRememberMeChecked && <Text style={styles.checkmark}>✓</Text>}
                </View>
