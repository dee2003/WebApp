        ...
        axios.get(`${API_URL}/users/role/foreman`)
            .then(foremenRes => {
                ...
                axios.get(`${API_URL}/crew-mapping/`)
                    .then(crewRes => {
                        const allCrews = crewRes.data;

                        // Identify foreman IDs that have at least one employee AND one equipment assigned
                        const eligibleForemanIds = allCrews
                            .filter(crew => 
                                (crew.employees && crew.employees.length > 0) && 
                                (crew.equipment && crew.equipment.length > 0)
                            )
                            .map(crew => crew.foreman_id);

                        // Filter the foremen list
                        const filteredForemen = allForemen.filter(fm => 
                            eligibleForemanIds.includes(fm.id)
                        );
                        
                        setForemen(filteredForemen);
                    })
                    .catch(crewErr => {
                        console.error("Failed to load crew mappings for filtering:", crewErr);
                        // Fallback: If crew data fails, show all foremen (or none, depending on desired strictness)
